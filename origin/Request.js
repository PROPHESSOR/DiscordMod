...
var _AnalyticsBridge = require('./AnalyticsBridge');

var _AnalyticsBridge2 = _interopRequireDefault(_AnalyticsBridge);
...
function requestWithMethod(method, origOpts, origCallback) {
  ...
  function track(args) {
    _AnalyticsBridge2.default.track('strict_ssl_request_attempt', args);
  }
  ...
  function attempt(index) {
    ...
    factory().on('response', function (response) {
      track({ success: true, method: method });
      ...
      if (index + 1 < requestTypes.length) {
        ...
        track({ success: false, method: 'all' });
        rv.emit('error', error);
      }
    });
  }
